...
from restApi.serializers import OperadorSerializer, AsistenciaSerializer, AdminSerializer, TipoUsuarioSerializer, ProyectoSerializer,ProyectoOperadorSerializer
...
class EnviarReporteOperador(APIView):
    ...
    def post(self, request):
        ...
        # Consulta a la base. Obtener las asistencias que caen en el intervalo de interes
        asistencias = Asistencia.objects.filter(fecha__gte = fechaInicio).filter(fecha__lte = fechaFin).filter(idOperador = idOperador)
...
class ProyectoList(APIView):
    ...
    def post(self, request, format=None):
        serializer = ProyectoSerializer(data=request.data, many=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ProyectoOperadorList(APIView):

    def get(self, request, format=None):
        proyecto = ProyectoOperador.objects.all()
        serializer = ProyectoOperadorSerializer(proyecto, many=True)
        return Response(serializer.data)

    def post(self, request, format=None):
        serializer = ProyectoOperadorSerializer(data=request.data, many=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ProyectoOperadorDetail(APIView):

    def get_object(self, pk):
        try:
            return ProyectoOperador.objects.get(pk=pk)
        except ProyectoOperador.DoesNotExist:
            raise Http404

    def get(self, request, pk, format=None):
        proyecto = self.get_object(pk)
        serializer = ProyectoOperadorSerializer(proyecto)
        return Response(serializer.data)

    def put(self, request, pk,  format=None):
        proyecto = self.get_object(pk)
        serializer = ProyectoOperadorSerializer(proyecto, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
